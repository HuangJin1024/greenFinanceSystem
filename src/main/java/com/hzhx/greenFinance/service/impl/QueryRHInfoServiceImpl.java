...
import com.hzhx.greenFinance.dao.QueryRHCLInfoMapper;
import com.hzhx.greenFinance.dao.QueryRHInfoMapper;
import com.hzhx.greenFinance.renHangVo.Customer;
import com.hzhx.greenFinance.renHangVo.CustomersLoansReport;
...
import com.hzhx.greenFinance.renHangVo.ReportSummaryCl;
...
public class QueryRHInfoServiceImpl implements QueryRHInfoService {
	...
	@Autowired
	private QueryRHCLInfoMapper queryRHCLInfoMapper;
	...
	@Override
	public void queryMCNInfoToXml(String fileName) {
		createDir();

		xmlFileName = fileName.substring(0, 26) + "_001" + ".xml";
		getCustomersLoansReport(xmlFileName);
		...
		try {
			FileOutputStream fos;
			fos = new FileOutputStream(new File(zipPath + fileName));
			ZipUtils.toZip(fileList, fos);
			logger.info("---将xml打包zip成功---");
		} catch (FileNotFoundException e) {
			e.printStackTrace();
	...
	private void getCustomersLoansReport(String xmlFileName) {
		CustomersLoansReport clReport = new CustomersLoansReport();
		
		ReportSummaryCl rscl = new ReportSummaryCl();
		int clNum = queryRHCLInfoMapper.queryCustomersLoanNum();
		rscl.setCustomerNumber(clNum);
		rscl.setDataDate(DateUtils.formatDate(new Date(), "yyyyMMdd"));
		rscl.setReportBank(env.getProperty("reportBankCd_RH"));
		
		List<Customer> customers = queryRHCLInfoMapper.queryCustomersLoans();
		
		clReport.setReportSummary(rscl);
		clReport.setCustomers(customers);
		creatXmlFile(clReport, xmlFileName);
	}
}
