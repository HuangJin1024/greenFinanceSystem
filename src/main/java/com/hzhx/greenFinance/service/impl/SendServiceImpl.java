...
public class SendServiceImpl implements SendService {
	...
	public ResultVo reportData(String msgType, String msgBusinessType) {
		...
		HashMap<String, String> httpParamMap;
		try {
			httpParamMap = (HashMap<String, String>) httpParamUtil.reportDataParam(msgType, msgBusinessType);
		} catch (Exception e) {
			resultVo.setCode("999");
			resultVo.setMessage("文件异常，未调用接口");
			return resultVo;
		}
		...
		/*
		 * 上传文件时需要 FileSystemResource fileSystemResource = new
		 ...
		 * HttpEntity<MultiValueMap<String, Object>>(paramMap, headers);
		 */
	...
	public ResultVo reportForm(String formPeriod) {
		HashMap<String, String> httpParamMap;
		try {
			httpParamMap = (HashMap<String, String>) httpParamUtil.reportFormParam(formPeriod);
		} catch (Exception e) {
			resultVo.setCode("999");
			resultVo.setMessage("文件异常，未调用接口");
			return resultVo;
		...
		Map<String, Object> map = new HashMap<String, Object>();
		// 文件名
		map.put("formPeriod", httpParamMap.get("formPeriod"));
		// 文件名
		map.put("fileName", httpParamMap.get("fileName"));
		// 报送文件串
		map.put("fileBase64", httpParamMap.get("fileBase64"));
	...
	public ResultVo reportDataAll(String msgType) {
		...
		HashMap<String, String> httpParamMap;
		try {
			httpParamMap = (HashMap<String, String>) httpParamUtil.reportAllDataParam(msgType);
		} catch (Exception e) {
			resultVo.setCode("999");
			resultVo.setMessage("文件异常，未调用接口");
			return resultVo;
		...
		greenfinSendLog.setFileNameIndex(httpParamMap.get("fileName").substring(0, 1));
		greenfinSendLog.setRequestType(httpParamMap.get("fileName").substring(26, 29));
		try {
			if ("true".equals(httpOpen)) {
				response = restTemplate.postForObject(httpParamMap.get("url"), httpEntity, String.class);
			}
	...
	public ResultVo reportDataDaily(String msgType, String dataDate) {
		...
		HashMap<String, String> httpParamMap;
		try {
			httpParamMap = (HashMap<String, String>) httpParamUtil.reportDataDailyParam(msgType, dataDate);
		} catch (Exception e) {
			resultVo.setCode("999");
			resultVo.setMessage("文件异常，未调用接口");
			return resultVo;
